...
LExit:
        ...
        )
    {
		...
		dwBool = 0;
		hr = XmlGetAttributeNumber(pNode, L"LaunchPassive", &dwBool);
		if (E_NOTFOUND == hr)
		{
			hr = S_OK;
		}
		...
		{
			m_fLaunchPassive = 0 < dwBool;
		}
		BalExitOnFailure(hr, "Failed to get LaunchPassive value.");

		dwBool = 0;
		hr = XmlGetAttributeNumber(pNode, L"LaunchQuiet", &dwBool);
		if (E_NOTFOUND == hr)
		{
			hr = S_OK;
		}
		...
		{
			m_fLaunchQuiet = 0 < dwBool;
		}
		BalExitOnFailure(hr, "Failed to get LaunchQuiet value.");
...
#pragma warning(suppress:4244)
        ...
        )
    {
            ...
            (WIXSTDBA_STATE_APPLIED == m_state && m_fPrereq))
        {
			...
			if ((BOOTSTRAPPER_DISPLAY_PASSIVE == m_command.display && m_fLaunchPassive) || (m_command.display == BOOTSTRAPPER_DISPLAY_NONE && m_fLaunchQuiet))
			{
				BOOL fLaunchTargetExists = BalStringVariableExists(WIXSTDBA_VARIABLE_LAUNCH_TARGET_PATH);
				if (fLaunchTargetExists)
					OnClickLaunchButton();
			}
